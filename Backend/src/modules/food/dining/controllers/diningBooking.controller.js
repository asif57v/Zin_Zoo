...
export async function getPublicRestaurantBookings(req, res, next) {
    try {
        const { restaurantId } = req.params;
        const bookings = await diningService.getRestaurantDiningBookingsPublic(restaurantId);
        res.status(200).json({ success: true, message: 'Public bookings fetched successfully', data: bookings });
    } catch (error) {
        next(error);
    }
}
