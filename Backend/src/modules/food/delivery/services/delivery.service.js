...
export const updateDeliveryPartnerProfile = async (userId, payload, files) => {
    ...
    if (drivingLicenseNumber !== undefined && drivingLicenseNumber !== partner.drivingLicenseNumber) {
        partner.drivingLicenseNumber = drivingLicenseNumber;
        updatedDocsRequiringReapproval = true;
    }
    if (panNumber !== undefined && panNumber !== partner.panNumber) {
        partner.panNumber = panNumber;
        updatedDocsRequiringReapproval = true;
    }
    if (aadharNumber !== undefined && aadharNumber !== partner.aadharNumber) {
        partner.aadharNumber = aadharNumber;
        updatedDocsRequiringReapproval = true;
    }
    ...
    if (updatedDocsRequiringReapproval || partner.status !== 'approved') {
    ...
    try {
        const { notifyAdminsSafely } = await import('../../../../core/notifications/firebase.service.js');
        void notifyAdminsSafely({
            title: 'Delivery Partner Joining Request 🚲',
            body: `Delivery partner "${partner.name}" has submitted/updated their onboarding documents and is pending verification.`,
            data: {
                type: 'new_registration',
                subType: 'delivery_partner',
                id: String(partner._id)
        ...
        // eslint-disable-next-line no-console
        console.error('Failed to notify admins of delivery partner document submission:', e);
    }
...
export const updateDeliveryPartnerBankDetails = async (userId, payload, files) => {
    ...
    if (bankDetails) {
        const b = bankDetails;
        if (b.accountHolderName !== undefined) partner.bankAccountHolderName = b.accountHolderName ? String(b.accountHolderName).replace(/\s+/g, ' ').trim() : '';
        ...
        if (b.bankName !== undefined) partner.bankName = b.bankName ? String(b.bankName).replace(/\s+/g, ' ').trim() : '';
