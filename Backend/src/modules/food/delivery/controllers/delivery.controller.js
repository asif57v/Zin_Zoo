...
import { verifyReferralCode } from '../../shared/services/mlm.service.js';
...
export const validateExternalReferralCodeController = async (req, res, next) => {
    try {
        ...
        const dummyCode = String(process.env.REFERRAL_DUMMY_CODE || '123456').trim();
        if (code === dummyCode) {
            return sendResponse(res, 200, 'Referral code is valid', { valid: true, source: 'dummy' });
        }

        const shouldCallExternalApi =
            String(process.env.ENABLE_EXTERNAL_REFERRAL_API || 'true').toLowerCase() === 'true';
        if (!shouldCallExternalApi) {
            return res.status(400).json({
                success: false,
                message: 'Invalid referral code',
                data: { valid: false, source: 'local' }
            });
        }
        ...
        if (result.valid) {
            return sendResponse(res, 200, 'Referral code is valid', {
                valid: true,
                source: 'external',
        ...
        return res.status(400).json({
            success: false,
            message: 'Invalid referral code',
            data: { valid: false, source: 'external', error: result.error || null }
        });
    } catch (error) {
        next(error);
    }
