...
export const createRestaurantSupportTicketController = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const restDoc = await FoodRestaurant.findById(restaurantId).select('restaurantName').lean();
            const restName = restDoc?.restaurantName || 'A restaurant';

            await notifyAdminsSafely({
                title: 'New Restaurant Support Ticket 📩',
                body: `${restName} raised a ticket: "${issueType}"`,
                data: {
            ...
            console.error('Failed to notify admins of new restaurant support ticket:', pushError);
        }
