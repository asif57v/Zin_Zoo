...
export default function PromoRow({ handleVegModeChange, navigate, isVegMode, toggleRef }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2.5 px-4 pt-2 pb-3 bg-white dark:bg-[#0a0a0a]">
      {promoCardsData.map((promo, idx) => (
        <div
          key={idx}
          ref={promo.id === 'pure-veg' ? toggleRef : null}
          className={`${promo.bgColor} ${promo.borderColor} rounded-[1.5rem] p-1 flex flex-col items-center h-[96px] shadow-sm border transition-all duration-300 cursor-pointer active:scale-95 group relative overflow-hidden ${
        ...
        >
          ...
          <div className="py-1 px-1 flex flex-col items-center text-center relative z-10 w-full">
            <span className="text-[8px] font-black text-gray-400/80 dark:text-gray-500 tracking-[0.15em] uppercase leading-none mb-0.5">
              {promo.title}
            </span>
            <div className={`text-[10px] sm:text-[11px] font-black ${promo.textColor} leading-none truncate w-full px-1 flex items-center justify-center gap-0.5`}>
              {promo.value}
            </div>
          </div>

          <div className={`flex-1 w-full ${promo.iconContainerColor} backdrop-blur-sm rounded-[1.1rem] flex items-center justify-center p-1.5 mt-auto mb-1 overflow-hidden relative shadow-inner`}>
            <img
              src={promo.icon}
              alt={promo.value}
